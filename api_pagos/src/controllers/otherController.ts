...
import { EarningsReport, IncomeExpenseReport, TransaccionReport, UserStatusReport } from '../repository/reporteRepository';
...
export const getReport5 = async (req: Request, res: Response): Promise<any> => {
    try {
        ...
        const payload: EarningsReport = {
            fecha_generacion: new Date().toISOString(),
            total_earnings: data.total_earnings,
            total_withdrawals: data.total_withdrawals,
        }
        const pdfBuffer = await generateTransactionPDF2_name_template(payload, 'reporte5');
        const timestamp = new Date().getTime();
        res.setHeader('Content-Type', 'application/pdf');
        res.setHeader('Content-Disposition', `attachment; filename="transaction-${timestamp}.pdf"`);
        res.end(pdfBuffer);
